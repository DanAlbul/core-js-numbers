...
function roundToPowerOfTen(num, pow) {
  const fract = num / 10 ** pow;
  return parseInt(`${Math.round(fract)}${'0'.repeat(pow)}`, 10);
}
 ...
 *   1  => 1
 */
function getSumToN(end) {
  return Array.from({ length: end - 0 + 1 }, (_, i) => 0 + i).reduce(
    (a, c) => a + c,
    0
  );
}
...
function toPrecision(number, precision) {
  return parseFloat(number.toPrecision(precision)).toFixed(
    Math.max(0, precision - Math.floor(Math.log10(Math.abs(number))) - 1)
  );
}
/**
...
function getNumberValue(number) {
  if (typeof number === 'object') return number.valueOf();
  return number;
}
...
function roundToSmallestInteger(number) {
  return number > 0 ? Math.ceil(number) - 1 : Math.floor(number);
}
...
function roundToLargestInteger(number) {
  return number > 0 ? Math.ceil(number) : Math.floor(number) + 1;
}
